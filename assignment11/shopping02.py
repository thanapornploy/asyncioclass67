...
async def main():
    ...
    QUEUE = 3
    CASHIER = 5
    customer_queue = Queue(QUEUE) #queue
    customers_start_time = time.perf_counter()
    customer_producer = asyncio.create_task(customer_generation(customer_queue, CUSTOMER)) #customer
    cashiers = [checkout_customer(customer_queue, i) for i in range(CASHIER)] #cashier
